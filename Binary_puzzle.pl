...
%   Devolve: R -> o triplo resultante de aplicar a Regra 1 do jogo ao Triplo,
%           isto e, nao existem 3 zeros ou 3 uns seguidos em nenhuma linha 
%           ou coluna.
%-----------------------------------------------------------------------------------
aplica_R1_triplo(Lista, R) :- Lista = [Y,X,Z], var(Y),var(Z), number(X), !, R = Lista.
aplica_R1_triplo(Lista, R) :- Lista = [Y,Z,X], var(Y),var(Z), number(X), !, R = Lista.
aplica_R1_triplo(Lista, R) :- Lista = [X,Y,Z], var(Y),var(Z), number(X), !, R = Lista.
aplica_R1_triplo(Lista, R) :- Lista = [X,Y,Z], var(X),var(Y),var(Z),!, R = Lista.
...
%   Devolve: N_Fila -> o resultante de aplicar a Regra 1 a Fila ate nao ser
%           possivel alterar mais nenhuma posicao.
%-----------------------------------------------------------------------------------
aplica_R1_fila(Fila, N_Fila) :-
...
%   Devolve: N_Puz -> o resultante de propagar as posicoes alteradas em cada
%           alteracao feita.
%-----------------------------------------------------------------------------------
